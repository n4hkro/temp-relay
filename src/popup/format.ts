...
import { StatusLevel } from "../shared/state";
import type { Status, StatusLevelValue } from "../shared/state";

export const EMPTY_STATUS_TEXT = "—";

// status null ola bilər: extension təzə qurulanda hələ heç bir hadisə yazılmayıb
export function formatStatusText(status: Status | null | undefined): string {
...
export const statusLevel = (status: Status | null | undefined): StatusLevelValue =>
...
export function statusTimeLeft(status: Status | null | undefined, now: number = Date.now()): number {
...
export const statusExpired = (status: Status | null | undefined, now: number = Date.now()): boolean =>
  statusTimeLeft(status, now) === 0;
