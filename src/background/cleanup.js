...
//      (shared/cleanup.js → tabCleanupPlan), yəni istənilən sayt üçün işləyir.
//
...
import { DATA_TYPES, ORIGIN_TYPES } from "../shared/cleanup.js";
import { registrableDomain } from "../shared/domains.js";

const cookieKey = (c) => [c.domain, c.name, c.path, c.storeId, JSON.stringify(c.partitionKey ?? null)].join("|");
...
async function collectCookies(cleanup) {
  const found = [];
  ...
  for (const domain of cleanup.cookieDomains ?? []) {
    try {
      ...
      console.warn("cookie-lər sadalanmadı:", domain, e?.message ?? e);
    }
  }
  for (const topLevelSite of cleanup.partitionTopLevelSites ?? []) {
    try { found.push(...await chrome.cookies.getAll({ partitionKey: { topLevelSite } })); } catch (e) {
      console.warn("partitioned cookie-lər sadalanmadı:", topLevelSite, e?.message ?? e);
    }
  }
  return dedupe(found);
}
...
async function removeCookies(cookies) {
  ...
  for (const c of cookies) {
    ...
    try {
      ...
      console.warn("cookie silinmədi:", c.domain, c.name, e?.message ?? e);
    }
  }
  return removed;
}
...
// `plan` — relay deskriptorunun `cleanup` sahəsi və ya tabdan törədilən plan; formaları eynidir.
export async function clearData(plan = {}) {
  const removed = await removeCookies(await collectCookies(plan));
  const origins = plan.storageOrigins ?? [];
  if (origins.length) await chrome.browsingData.remove({ origins, originTypes: ORIGIN_TYPES }, DATA_TYPES);
  return { cookies: removed };
}

export const clearRelayData = (relay) => clearData(relay.cleanup ?? {});
...
export async function clearPageStorage(tabId, allowedOrigins = []) {
  try {
    ...
    console.warn("səhifə saxlancı təmizlənmədi:", e?.message ?? e);
    return 0;
  }
}
