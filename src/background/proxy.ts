...
//
// Bölgü: sətirin NECƏ oxunduğu — `shared/proxy.ts` (saf, testlənir); chrome ilə iş buradadır.
//
...
//   • yerli ünvanlar bypass siyahısındadır (shared/proxy.ts → BYPASS);
...
import {
  ...
  type InvalidProxyLine, type ProxyControl, type ProxyEntry,
} from "../shared/proxy";
import {
...
let proxyMutations: Promise<unknown> = Promise.resolve();
function mutateProxy<T>(job: () => Promise<T>): Promise<T> {
  const result: Promise<T> = proxyMutations.then(job);
...
const isCurrentLifecycle = (version: number): boolean => version === lifecycleVersion;
const cancelledExit = (): ExitCheck => ({ ok: false, cancelled: true, error: "yoxlama ləğv edildi" });
...
): Promise<{ added: number; replaced: number; duplicates: number; invalid: InvalidProxyLine[]; total: number }> {
...
// --- brauzerin parametri --------------------------------------------------------------------
// Saxlancdaki qeyd ilə brauzerin həqiqi parametri ayrı şeylərdir (bax: shared/proxy.ts →
...
export async function browserProxy(): Promise<ProxyControl> {
  try {
    return readProxyControl(await chrome.proxy.settings.get({ incognito: false }));
  } catch (e) {
    console.warn("brauzerin proxy parametri oxunmadı:", (e as { message?: unknown })?.message ?? e);
...
const sameEndpoint = (
  left: EndpointLike | null | undefined,
  right: EndpointLike | null | undefined,
...
const effectiveSingleProxy = (details: unknown): EndpointLike | null => {
  const d = details as {
    value?: { mode?: unknown; rules?: { singleProxy?: EndpointLike | null } | null } | null;
  } | null | undefined;
  return d?.value?.mode === "fixed_servers"
    ? d.value.rules?.singleProxy ?? null
    : null;
};

const ownsEffectiveProxy = (details: unknown, selected: ProxyEntry): boolean => {
  const d = details as { levelOfControl?: unknown } | null | undefined;
  return d?.levelOfControl === "controlled_by_this_extension"
    && sameEndpoint(effectiveSingleProxy(details), selected);
};

async function readProxyDetails(): Promise<unknown> {
  return chrome.proxy.settings.get({ incognito: false });
}

async function verifyAppliedProxy(selected: ProxyEntry): Promise<unknown> {
  let details: unknown;
  try {
    details = await readProxyDetails();
  } catch (e) {
    throw new ExpectedError(`Chrome proxy parametrini yoxlamaq mümkün olmadı: ${(e as { message?: unknown })?.message ?? e}`);
  }
...
async function releaseProxyNow(): Promise<ProxyControl> {
  const clear = async (): Promise<boolean> => {
    try { await chrome.proxy.settings.clear({ scope: "regular" }); return true; } catch (e) {
      console.warn("proxy parametri buraxılmadı:", (e as { message?: unknown })?.message ?? e);
      return false;
    }
  };
  ...
  try {
    ...
    console.warn("birbaşa bağlantı məcbur edilmədi:", (e as { message?: unknown })?.message ?? e);
  }
  await clear();
...
export function releaseProxy(): Promise<ProxyControl> {
  return mutateProxy(releaseProxyNow);
}

// --- qoşulma --------------------------------------------------------------------------------
const findProxy = async (key: string): Promise<ProxyEntry | null> => {
  const { list } = await readProxies();
...
export async function connectProxy(key: string): Promise<{ proxy: ProxyEntry | null; exit: ExitCheck }> {
  const version = beginLifecycle();
  const setup = await mutateProxy(async (): Promise<ConnectSetup | null> => {
    ...
    await chrome.proxy.settings.set({ value: proxyConfig(selected) as chrome.proxy.ProxyConfig, scope: "regular" });
    let details: unknown;
    try {
  ...
  // `controller` mutateProxy-nin uğurlu yolunda həmişə qurulur (uğursuzluq yuxarıda
  // `cancelled` ilə ələ keçir) — `as` yalnız tip üçündür, runtime orijinaldır.
  let exit: ExitCheck = await checkExit((setup.controller as AbortController).signal);
  const ping = Date.now() - started;
  await mutateProxy(async (): Promise<void> => {
    ...
    try {
      ...
      exit = { ok: false, error: (((e as { message?: unknown })?.message ?? String(e)) as string) };
      await clearActiveProxy();
...
async function markProxyNow(key: string, patch: Partial<ProxyEntry>): Promise<void> {
  const { list } = await readProxies();
  let changed = false;
  const next = list.map((proxy) => {
    if (proxyKey(proxy) !== key) return proxy;
    changed = true;
    const updated: ProxyEntry = { ...proxy, ...patch, checked: Date.now() };
...
async function disconnectProxyNow(): Promise<ProxyControl> {
...
export function disconnectProxy(): Promise<ProxyControl> {
  beginLifecycle();
  return mutateProxy(disconnectProxyNow);
}

// Sorğu brauzerin cari proxy parametrindən keçir — yəni bu, tunelin işlədiyini yoxlayır.
export async function checkExit(cancellation: AbortSignal | null = null): Promise<ExitCheck> {
  const controller = new AbortController();
  const forward = (): void => controller.abort(cancellation!.reason);
  ...
  try {
    ...
    const request = (async (): Promise<ExitCheck> => {
    ...
    return {
      ok: false,
      error: (e as { name?: unknown })?.name === "TimeoutError"
        ? "vaxt bitdi"
        : (((e as { message?: unknown })?.message ?? String(e)) as string),
    };
  } finally {
    clearTimeout(timer);
...
export async function resumeProxy(): Promise<void> {
  ...
  const setup = await mutateProxy(async (): Promise<ResumeSetup | null> => {
    ...
    try {
      ...
      if (problem) throw new ExpectedError(problem);
      await chrome.proxy.settings.set({ value: proxyConfig(active) as chrome.proxy.ProxyConfig, scope: "regular" });
      await verifyAppliedProxy(active);
    } catch (e) {
      console.warn("proxy bərpa olunmadı:", (e as { message?: unknown })?.message ?? e);
      await clearActiveProxy();
      try {
  ...
  await mutateProxy(async (): Promise<void> => {
    ...
    try {
      ...
      await clearActiveProxy();
      if ((await browserProxy()).ours) await releaseProxyNow();
      await writeStatus(StatusLevel.warn, `Proxy bərpa olunmadı: ${(e as { message?: unknown })?.message ?? e}`);
      return;
    }
...
function takeAuthAttempt(key: string): boolean {
  const count = authAttempts.get(key) ?? 0;
  if (count >= MAX_AUTH_ATTEMPTS) return false;
  authAttempts.delete(key);
  authAttempts.set(key, count + 1);
  while (authAttempts.size > AUTH_ATTEMPT_CACHE) authAttempts.delete(authAttempts.keys().next().value as string);
  return true;
}

export function answerProxyAuth(
  details: chrome.webRequest.OnAuthRequiredDetails,
  callback?: (response: chrome.webRequest.BlockingResponse) => void,
): chrome.webRequest.BlockingResponse | undefined {
  if (!details?.isProxy) { callback!({}); return; }
  mutateProxy(async (): Promise<chrome.webRequest.BlockingResponse> => {
    const active = await readActiveProxy();
...
export async function migrateProxies(): Promise<void> {
  return mutateProxy(async (): Promise<void> => {
