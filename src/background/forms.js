...
//   • dəyərlər TƏSADÜFİ DEYİL: `shared/identity.js` bütöv, uzlaşan şəxs profili verir
...
import { identityFamily } from "../shared/cleanup.js";
import { isLocalHost, registrableDomain } from "../shared/domains.js";
import { isLiveSession } from "../shared/state.js";
import { getTab, readDocument, settleTab, waitForDocument } from "./tabs.js";
...
// MV3 worker-i ~30 s boşluqdan sonra sönür (eyni üsul: background/inbox.js → pulse)
...
export const openSignupInPage = async (timeoutMs, options) => {
  ...
  const has = (haystack, list) => list.some((w) => {
    if (!patterns.has(w)) patterns.set(w, new RegExp(`(^| )${word(w)}( |$)`));
    return word(w) !== "" && patterns.get(w).test(haystack);
  });
  // Kök üzrə uyğunluq: "subscribe/subscription", "abunə/abunəlik", "рассылка/рассылки"
  const hasStem = (haystack, list) => list.some((w) => haystack.includes(word(w)));
  const attr = (el, name) => el?.getAttribute?.(name) ?? null;
  const usable = (el) => !!(el && !el.disabled && (el.offsetParent || el.offsetWidth || el.offsetHeight));
  const label = (el) => ((el.textContent ?? "") || el.value || attr(el, "aria-label") || "")
    .replace(/\s+/g, " ").trim();
  ...
  const routeOf = (url) => (url ? (url.pathname + url.search + url.hash).toLowerCase() : "");
  const onSignupRoute = () => SIGNUP_PATH.test(routeOf(here()));
  ...
  const sigOf = (el) => norm([attr(el, "autocomplete"), el.name, el.id, el.placeholder,
    attr(el, "aria-label"), attr(el, "title")].filter(Boolean).join(" "));

  const subscribeish = (text) => hasStem(text, SUBSCRIBE_STEMS) || has(text, SUBSCRIBE);
  const isPassword = (el) => norm(el.type) === "password";
  const isCodeField = (el) => has(sigOf(el), CODE_WORDS);
  const isProfileField = (el) => has(sigOf(el), ["username", "user name", "nickname", "first name",
    "last name", "full name", "name", "phone", "telefon", "istifadeci"]);

  // Qutuda qeydiyyat əlaməti var? (parol, kod, ad/istifadəçi adı xanası)
  const boxHasSignupField = (box) => fieldsIn(box).some((el) => isPassword(el) || isCodeField(el) || isProfileField(el));
  ...
  const signupSubmit = (root) => {
    if ((root?.tagName ?? "").toUpperCase() !== "FORM") return false;
    try {
      return [...root.querySelectorAll(BUTTONS)].some((el) => (el.offsetParent || el.offsetWidth || el.offsetHeight)
        && norm(el.type) === "submit" && attr(el, "role") !== "tab"
        && has(norm(label(el)), OPEN) && !has(norm(label(el)), BANNED));
    } catch { return false; }
  };

  const visibleText = (root, limit = 600) => {
    let out = "";
    try {
  ...
  const isSelected = (el) => attr(el, "aria-selected") === "true"
  ...
  const signupFormOpen = () => {
    ...
    const settled = (el) => !inLoginPanel(el) && !pendingSignupTab(boxOf(el));
    ...
    return Boolean(box) && has(namesIn(box), OPEN);
  };
  ...
  const signupLinks = () => {
    ...
    for (const link of links) {
      let target = null;
      try { target = new URL(attr(link, "href"), url.href); } catch { continue; }
  ...
  const isFormSubmit = (el) => {
    try {
      const control = ["BUTTON", "INPUT"].includes((el.tagName ?? "").toUpperCase())
  ...
  const wasClicked = (el, skip) => [...skip].some((clicked) => clicked === el
  ...
  const collectLoose = (scope, skip, seen, out) => {
    let all = [];
  ...
  const waitForStep = (limit, read) => new Promise((resolve) => {
    const initial = read();
...
export const fillFormInPage = async (values, options) => {
  if (options?.allowedOrigins ? !options.allowedOrigins.includes(location.origin)
  ...
  const hasStem = (haystack, list) => list.some((w) => haystack.includes(norm(w)));
  const attr = (el, name) => el.getAttribute?.(name) ?? null;
  ...
  const visible = (el) => !!(el && (el.offsetParent || el.offsetWidth || el.offsetHeight));
  const usable = (el) => !!el && !el.disabled && !el.readOnly && visible(el);
  ...
  const buttonText = (el) => (((el.value && (el.tagName ?? "").toUpperCase() === "INPUT" ? el.value : el.textContent) ?? "")
  ...
  const ownerForm = (el) => { try { return el.closest?.("form") ?? null; } catch { return null; } };
  const subscribeish = (text) => hasStem(text, SUBSCRIBE_STEMS) || has(text, SUBSCRIBE);
  ...
  const panelKind = (box) => {
    if (!box) return "";
    if (panelCache.has(box)) return panelCache.get(box);
    const kind = computePanelKind(box);
    panelCache.set(box, kind);
    return kind;
  };

  const computePanelKind = (box) => {
    if (signupSubmit(box)) return "signup";
  ...
  const signupSubmit = (root) => {
    if ((root?.tagName ?? "").toUpperCase() !== "FORM") return false;
    try {
      return [...root.querySelectorAll(BUTTON_SELECTOR)].some((el) => visible(el)
        && norm(el.type) === "submit" && attr(el, "role") !== "tab"
  ...
  const isConsent = (t) => has(t, CONSENT) || hasStem(t, CONSENT_STEMS);
  const isMarketing = (t) => has(t, MARKETING) || hasStem(t, MARKETING_STEMS);
  ...
  const optionText = (option) => norm(((option.textContent ?? "") + " " + (option.value ?? "")).replace(/\s+/g, " ").trim());
  const optionsOf = (el) => { try { return [...el.options]; } catch { return []; } };
  const isRequired = (el) => el.required === true || attr(el, "aria-required") === "true";
  ...
  const labelElement = (el) => {
    ...
    try { return el.closest?.("label") ?? null; } catch { return null; }
  };
  ...
  const fieldsOf = (root) => {
    if (blockedForms.has(root)) return null;      // abunə/giriş/axtarış forması seçilməsin
    let all = [];
    try { all = [...root.querySelectorAll("input, select, textarea")]; } catch { return null; }
    const out = [];
    for (const el of all) {
      const kind = kindOf(el);
  ...
  const nearBy = (first, second) => containersOf(first).slice(0, 4).some((box) => {
  ...
  const contextBonus = (root, map) => {
    if (root === document) return 0;
    let bonus = 0;
    try {
      const dialog = root.closest?.("[role='dialog'], [aria-modal='true'], .modal");
  ...
  const signupContext = (map, fields, root) => {
  ...
  const tightest = (fields, evaluate) => {
    const roots = [];
    for (const { el, kind } of fields) {
      if (kind === "checkbox" || kind === "radio") continue;   // razılıq xanası kök törətmir
      for (const box of containersOf(el)) {
        if (box === document || roots.includes(box)) continue;
    ...
    const coherent = scored.filter((cand) => {
      ...
      return (cand.map.has("email") || cand.map.has("code"))
    ...
    const dialogs = coherent.filter((cand) => cand.root.tagName === "DIALOG"
      || cand.root.getAttribute?.("role") === "dialog" || cand.root.getAttribute?.("aria-modal") === "true");
    ...
    const nested = coherent.filter((cand) => !coherent.some((other) => other !== cand
      && cand.root.contains?.(other.root) === true
      && ["password", "passwordConfirm", "code"].every((role) => !cand.map.has(role) || other.map.has(role))));
    if (nested.length) {
    ...
    for (const cand of scored) {
      ...
      try { if (pick.root.contains?.(cand.root) === true) pick = cand; } catch { /* keç */ }
    }
    return pick;
  };
  ...
  const watch = (find, limit) => new Promise((resolve) => {
    const now = find();
    ...
    const check = () => { const hit = find(); if (hit) finish(hit); };
  ...
  const consentBoxes = () => {
    ...
    const add = (el) => {
      ...
      if (owner && (blockedForms.has(owner)
          || (root !== document && owner !== root && !root.contains?.(owner)))) return;
      if (root !== document && owner !== root && !root.contains?.(el)) {
  ...
  const consentKind = (el) => {
    ...
    const near = visible(el) || (root !== document && root.contains?.(el) === true);
  ...
  if (mode !== "submit") {
    ...
    for (const { el, kind } of fields) {
      ...
      const key = el.name || attr(el, "aria-labelledby") || "radio";
    ...
    for (const [, items] of groups) {
      ...
      const label = (el) => norm([el.value, attr(el, "aria-label"), labelOf(el, { box: false })].filter(Boolean).join(" "));
  ...
  const codeStage = !haveCode && map.has("email");
  ...
  const enabled = (el) => !el.disabled && attr(el, "aria-disabled") !== "true";
  ...
  const allButtons = () => {
    try {
      return [...root.querySelectorAll('button, input[type="submit"], input[type="button"], [role="button"], a[href="#"]')];
    } catch { return []; }
  };
  ...
  const result = { done: true, filled, failed, checked, chosen, skipped, ...(codeButtons.length ? { codeButtons } : {}) };
  ...
  const invalid = (el) => el.validity?.valid === false || attr(el, "aria-invalid") === "true";
  for (const [role, { el, kind }] of map) {
  ...
  try {
    ...
    return { ...result, reason: "düymə basılmadı: " + (e?.message ?? e) };
  }
...
export const describePageInPage = () => {
  ...
  const report = { url: "", title: "", forms: [], fields: [], clickables: [], note: [] };
  ...
  try {
    report.forms = [...document.querySelectorAll("form")].slice(0, 10).map((el) => ({
      ...
      buttons: cut([...el.querySelectorAll('button, input[type="submit"], [role="button"]')]
        .map((b) => norm(b.textContent || b.value || attr(b, "aria-label"))).filter(Boolean).join(" | "), 120),
    }));
...
async function run(session, func, args, limitMs, {
  ...
  const sameSite = (candidate) => {
    try {
      ...
      const family = identityFamily(expectedSite.hostname);
      return ["http:", "https:"].includes(url.protocol)
        && (registrableDomain(url.hostname) === registrableDomain(expectedSite.hostname)
          || family && identityFamily(url.hostname)?.id === family.id)
        && (!isLocalHost(expectedSite.hostname) || url.origin === expectedSite.origin);
    } catch { return false; }
  };
  if (session.relaySite) {
    ...
    args = [args[0], { ...args[1], allowedOrigin }];
    if (!await isLiveSession(session)) return { error: "sessiya dəyişdi" };
  }
  ...
  try {
    const target = { tabId: session.relayTabId };
    if (documentId) target.documentIds = [documentId];
      ...
      for (const frame of frames ?? []) {
        if (frame.documentId && sameSite(frame.result)) trustedOrigins.set(frame.documentId, frame.result);
      }
      if (!trustedOrigins.size) return { error: "eyni sayta aid əlçatan frame tapılmadı" };
      if (!await isLiveSession(session)) return { error: "sessiya dəyişdi" };
      target.documentIds = [...trustedOrigins.keys()];
      args = [args[0], { ...args[1], allowedOrigins: [...new Set(trustedOrigins.values())] }];
    ...
    const navigated = previousDocument ? waitForDocument(session.relayTabId, {
      previousDocumentId: previousDocument.documentId, timeoutMs: limitMs, signal: navigationController.signal,
    }).then((state) => state ? "navigated" : new Promise(() => {})) : new Promise(() => {});
    ...
    const guard = new Promise((resolve) => { timer = setTimeout(() => resolve("timeout"), limitMs); });
    ...
    const frames = (Array.isArray(outcome) ? outcome : [])
      .filter((item) => item && item.result && typeof item.result === "object");
    ...
    return { result: chosen.result, frameId: chosen.frameId ?? 0, documentId: chosen.documentId ?? null,
      origin: trustedOrigins.get(chosen.documentId) ?? allowedOrigin };
  } catch (e) {
    ...
    if (NO_ACCESS.test(text)) return { error: ACCESS_HINT, blocked: true };
    return { error: text, previousDocument };
  } finally {
    clearInterval(beat);
...
export async function openSignup(session, {
  timeoutMs = OPEN_TIMEOUT_MS, graceMs = ANSWER_GRACE_MS, allowNavigate = true,
} = {}) {
  const attempt = (navigate) => run(session, openSignupInPage, [timeoutMs, { navigate }], timeoutMs + graceMs,
  ...
  if (!error && result.navigate) {
    ...
    if (!await isLiveSession(session)) return { error: "sessiya dəyişdi" };
    try {
      await chrome.tabs.update(session.relayTabId, { url });
    } catch (e) {
      return { error: `qeydiyyat səhifəsi açılmadı (${url}): ${e?.message ?? e}` };
    }
  ...
  if (changing || timeout) {
    await waitForDocument(session?.relayTabId, {
      previousDocumentId: previousDocument?.documentId, url: current?.url,
...
export async function fillForm(session, values, {
  ...
  if (!filling.error && filling.result.done !== true) {
    const wide = await run(session, fillFormInPage, fillArgs, timeoutMs + graceMs, {
      allFrames: true,
      pick: (item) => item.result?.done === true,
    });
    if (!wide.error) {
      filling = wide;
  ...
  const { submitted, reason, codeRequested, codeButtons } = clicking.result;
  return {
    ...result,
...
export const signupValues = (session, extra = {}) => ({
  ...(session?.identity ?? {}),
  email: session?.address ?? session?.identity?.email ?? "",
  username: session?.username ?? session?.identity?.username ?? "",
...
export async function describePage(tabId) {
  ...
  try {
    ...
    const text = e?.message ?? String(e);
    throw new ExpectedError(NO_ACCESS.test(text) ? ACCESS_HINT : text);
  }
...
export const filledNote = (filled = []) => filled.map((role) => LABELS[role] ?? role).join(" + ");
...
export function fillNote(result) {
  const parts = [];
