...
import { StatusLevel, readShield, writeShield, writeStatus } from "../shared/state";

export const TRACKER_RULESET_ID = "trackers";

export async function isShieldOn(): Promise<boolean> {
  try {
...
export async function setShield(on: unknown): Promise<boolean> {
...
export async function resumeShield(): Promise<void> {
  ...
  try {
    ...
    console.warn("qalxan bərpa olunmadı:", (e as { message?: unknown })?.message ?? e);
  }
}
