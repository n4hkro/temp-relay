...
import { usernameFromAddress } from "./username";
import type { RandomBytes } from "./username";
...
function seedOf(text: unknown): number {
  let hash = 0x811c9dc5;
...
export interface Identity {
  ...
  // Cins: sayt seçim tələb edirsə göstərici lazımdır; "demək istəmirəm" varsa forms.ts onu
  // üstün tutur (bax: background/forms.ts → GENDER_NEUTRAL).
  gender: string;
...
export function buildIdentity(
  ...
  { now = Date.now(), usernameMaxLength }: IdentityOptions = {},
): Identity {
  ...
  const randomBytes: RandomBytes = (count) =>
