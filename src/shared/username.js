...
const DIGITS = "23456789";           // səhv oxunan 0/1 yoxdur (shared/password.js ilə eyni qayda)
const LETTERS = "abcdefghijkmnopqrstuvwxyz";

const bytes = (count) => crypto.getRandomValues(new Uint8Array(count));

// Modulo meylsiz seçim (shared/password.js → pick ilə eyni üsul)
...
// `maxLength` xananın öz `maxlength` atributundan gələ bilər (forms.js onu ötürür).
// `random` testdə determinist funksiya ilə əvəzlənir.
export function usernameFromAddress(address, { maxLength = USERNAME_MAX, random = bytes } = {}) {
