...
import { PUBLIC_SUFFIX_RULES } from "./public-suffix-rules";

const rules: Set<string> = new Set(PUBLIC_SUFFIX_RULES);
export const isLocalHost = (host: unknown): boolean => String(host).startsWith("[")
  || /^\d{1,3}(\.\d{1,3}){3}$/.test(String(host)) || !String(host).includes(".");
