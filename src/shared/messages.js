...
export const Message = Object.freeze({
  ...
  start: (tempId, relayId, options, tabId = null) => ({ type: MessageType.START, tempId, relayId, options, tabId }),
  ...
  newAddress: (options) => ({ type: MessageType.NEW_ADDRESS, options }),
  ...
  clearTab: (tabId) => ({ type: MessageType.CLEAR_TAB, tabId }),
  copy: (text) => ({ target: OFFSCREEN_TARGET, type: MessageType.COPY, text }),

  // --- alətlər ---
  setShield: (on) => ({ type: MessageType.SET_SHIELD, on: on === true }),
  ...
  proxyAdd: (text) => ({ type: MessageType.PROXY_ADD, text }),
  ...
  proxyRemove: (keys) => ({ type: MessageType.PROXY_REMOVE, keys: Array.isArray(keys) ? keys : [keys] }),
  proxyConnect: (key) => ({ type: MessageType.PROXY_CONNECT, key }),
  proxyDisconnect: () => ({ type: MessageType.PROXY_DISCONNECT }),
  // Diaqnostika: yalnız tab id-si gedir, səhifəni worker özü oxuyur
  describePage: (tabId) => ({ type: MessageType.DESCRIBE_PAGE, tabId }),
});

export const isForOffscreen = (message) => message?.target === OFFSCREEN_TARGET;
...
// isə onu qırmızı xəta kimi yox, sarı xəbərdarlıq kimi göstərir (bax: shared/errors.js).
