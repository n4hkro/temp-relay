...
export const describeFailure = (e: unknown): string => {
  const err = e as Error | null | undefined;
  if (err?.name === "AbortError") return "sorğu vaxtı bitdi (sayt cavab vermədi)";
...
export const isExpected = (e: unknown): boolean =>
  e instanceof ExpectedError || (e as { name?: unknown } | null | undefined)?.name === "AbortError";
