...
// Təsadüfi bayt mənbəyi — testdə determinist funksiya ilə əvəzlənir
...
function index(n: number, random: RandomBytes): number {
  if (n <= 1) return 0;
...
function shuffle(chars: string[], random: RandomBytes): string[] {
...
export function generatePassword(length: number = PASSWORD_LENGTH, random: RandomBytes = bytes): string {
