// Poçt təhlili qatının testləri (src/shared/extract.ts).
...
const nexoraRelay = { id: "nexora", name: "Nexora", url: "https://nexora.example/register" };
const hints = mailHints(nexoraRelay);

const html = (body: any) : any => ({ body, bodyContentType: "html" });
...
describe("decodeEntities / htmlToText", () => {
  it("adlandırılmış və rəqəmli entity-ləri açır", () => {
    expect(decodeEntities("a &amp; b &#53; &#x41; &nbsp;")).toBe("a & b 5 A  ");
  });
  ...
  it("style/script bloklarını atır, sözləri birləşdirmir", () => {
    ...
    expect(text.includes("color")).toBe(false);
    expect(text).toMatch(/bir\niki/);
  });

  it("bodyContentType html deyilsə body olduğu kimi oxunur", () => {
    expect(messageText({ body: "salam <b>", bodyContentType: "text" })).toBe("salam <b>");
    expect(messageText(html("salam <b>"))).toBe("salam");
  });
});

describe("domen uyğunluğu", () => {
  it("registrableDomain son iki etiketi götürür", () => {
    expect(registrableDomain("mail.nexora.example")).toBe("nexora.example");
    expect(registrableDomain("NEXORA.EXAMPLE")).toBe("nexora.example");
  });
  ...
  it("çox etiketli ictimai şəkilçi kəsilmir (co.uk, com.tr, github.io…)", () => {
    ...
    expect(registrableDomain("www.firma.com.tr")).toBe("firma.com.tr");
    expect(registrableDomain("blog.user.github.io")).toBe("user.github.io");
    expect(registrableDomain("app.proje.vercel.app")).toBe("proje.vercel.app");
    expect(registrableDomain("bucket.s3.amazonaws.com")).toBe("bucket.s3.amazonaws.com");
  });

  it("adi domenlərdə davranış dəyişmir", () => {
    expect(registrableDomain("login.microsoftonline.com")).toBe("microsoftonline.com");
  ...
  it("hostMatches alt-domenləri də əhatə edir", () => {
    expect(hostMatches("no-reply.mail.nexora.example", "nexora.example")).toBe(true);
    expect(hostMatches("nonexora.example", "nexora.example")).toBe(false);
    expect(hostMatches("nexora.example.evil.example", "nexora.example")).toBe(false);
  });

  it("mailHints url-dən domen və açar söz törədir, ümumi sözləri atır", () => {
    expect(hints.domains).toStrictEqual(["nexora.example"]);
    expect(hints.keywords).toStrictEqual(["nexora"]);
  });
  ...
  it("mailHints deskriptordakı mail sahəsini üstün tutur", () => {
    ...
    expect(custom.domains).toStrictEqual(["mailer.example", "relay.example"]);
    expect(custom.keywords.includes("mailer")).toBeTruthy();          // domendən törəyən
    expect(custom.keywords.includes("relay example")).toBeTruthy();   // deskriptorda yazılan olduğu kimi qalır
    expect(custom.keywords.includes("ab")).toBe(false);    // 3 hərfdən qısa olan atılır
  });
});
...
describe("uyğunluq (relevance)", () => {
  it("saytın öz məktubu eşiyi keçir", () => {
    ...
    expect(score >= MIN_RELEVANCE, `xal ${score} azdır`).toBeTruthy();
    expect(reasons.length > 0).toBeTruthy();
  });
  ...
  it("yad saytın məktubu keçmir", () => {
    ...
    expect(relevance(foreign, hints).score < MIN_RELEVANCE).toBeTruthy();
  });
  ...
  it("göndərən yaddan çıxmış subdomendir, amma keçid sayta gedirsə məktub bizimdir", () => {
    ...
    expect(relevance(via, hints).score >= MIN_RELEVANCE).toBeTruthy();
  });
  ...
  it("mövzuda saytın adı kifayət edir", () => {
    ...
    expect(relevance(subject, hints).score >= MIN_RELEVANCE).toBeTruthy();
  });
});
...
describe("keçidlər", () => {
  it("yalnız ana səhifə keçidi olan xoş gəldin məktubu izləməni bitirmir", () => {
    ...
    expect(extractFromMessage(message, hints)).toBe(null);
  });
  it("entity-ləri açır, son punktuasiyanı kəsir, etibarsız ünvanı atır", () => {
    expect(cleanUrl("https://nexora.example/c?token=a&amp;b=1.")).toBe("https://nexora.example/c?token=a&b=1");
    expect(cleanUrl("ftp://nexora.example/")).toBe("");
    expect(cleanUrl("https://nexora .com/")).toBe("");
  });
  ...
  it("href və çılpaq ünvanların ikisini də yığır, təkrarı atır", () => {
    ...
    expect(harvestLinks(message).map((l) => l.url)).toStrictEqual(["https://nexora.example/a", "https://nexora.example/b"]);
  });

  it("aktivasiya keçidini reklam və unsubscribe-dan üstün tutur", () => {
    const link = bestLink(ACTIVATION, hints);
    expect(link!.url).toBe("https://nexora.example/confirm?token=a91fQ2&uid=7731");
    expect(link!.score >= 40).toBeTruthy();
  });
  ...
  it("izləyici host (click.*) aktivasiya keçidi sayılmır", () => {
    ...
    expect(bestLink(message, hints)).toBe(null);
  });
});

describe("aktivasiya kodu", () => {
  it("kod yalnız məktubun mövzusundadırsa da tapılır", () => {
    expect(findCode({ subject: "Your verification code: 839271", body: "Use the code above to sign up." })?.value).toBe("839271");
  });

  it("mövzu və gövdədə təkrarlanan kod bir uzun kod kimi birləşmir", () => {
    expect(findCode({ subject: "Your verification code: 4839", body: "4839 is your verification code." })?.value).toBe("4839");
  });
  it("adi aktivasiya məktubundan 6 rəqəmli kodu tapır", () => {
    expect(findCode(ACTIVATION)?.value).toBe("483920");
  });
  ...
  it("URL-in içindəki təsadüfi sətir kod sayılmır", () => {
    ...
    expect(findCode(message)).toBe(null);
  });
  ...
  it("il kimi görünən dəyər endirilir", () => {
    ...
    expect(findCode(year)).toBe(null);
    expect(findCode(real)?.value).toBe("202611");
  });
  ...
  it("açar söz yaxınlığı və HTML vurğusu xalı artırır", () => {
    ...
    expect(findCode(plain)?.value).toBe("918273");
  });
  ...
  it("hərf+rəqəm qarışığı da tapılır (rəqəmli seçim yoxdursa)", () => {
    ...
    expect(findCode(mixed)?.value).toBe("X9K2M4");
  });
  ...
  it("kod sözündən sonra nöqtəli-bloklu yazılış da oxunur", () => {
    ...
    expect(findCode(spaced)?.value).toBe("123456");
  });
});
...
describe("normalizeMessages", () => {
  it("zərf formasını da, çılpaq massivi də oxuyur", () => {
    ...
    expect(wrapped[0].id).toStrictEqual(bare[0].id);
  });
  ...
  it("təkrar id-ni atır, təzəni öndə saxlayır, çatışmayan sahəni boşaldır", () => {
    ...
    expect(list.map((m) => m.id)).toStrictEqual(["m1", "m0", "7"]);
    expect(list[2].subject).toBe("");
    expect(list[2].bodyContentType).toBe("text");
  });
  ...
  it("zərf pozulubsa boş massiv qaytarır, xəta atmır", () => {
    expect(normalizeMessages(null)).toStrictEqual([]);
    expect(normalizeMessages({ data: "sətir" })).toStrictEqual([]);
    expect(normalizeMessages({ totalReceived: 5 })).toStrictEqual([]);
  });
});
...
describe("extractFromMessage / pickFromMessages", () => {
  it("eyni saytın yenidən göndərilmiş kodu köhnə, daha çox nişanlı məktubdan üstündür", () => {
    ...
    expect(pickFromMessages([older, newer], hints)?.code).toBe("938271");
  });
  it("uyğun məktubdan həm kodu, həm keçidi verir", () => {
    const found = extractFromMessage(ACTIVATION, hints);
    expect(found!.code).toBe("483920");
    expect(found!.link).toBe("https://nexora.example/confirm?token=a91fQ2&uid=7731");
    expect(found!.subject).toMatch(/confirm your email/i);
    expect(found!.reasons.length > 0).toBeTruthy();
    expect(typeof found!.at).toBe("number");
  });

  it("uyğun olmayan məktub üçün null qaytarır", () => {
    expect(extractFromMessage({ from: "a@b.example", subject: "salam", body: "", bodyContentType: "text" }, hints)).toBe(null);
  });
  ...
  it("saytın məktubudur, amma nə kod nə keçid yoxdursa null", () => {
    ...
    expect(extractFromMessage(bare, hints)).toBe(null);
  });
  ...
  it("heç bir məktub uyğun gəlmirsə null", () => {
    expect(pickFromMessages([], hints)).toBe(null);
    expect(pickFromMessages([{ from: "x@y.example", subject: "reklam", body: "", bodyContentType: "text" }], hints)).toBe(null);
  });
  ...
  it("kod yoxdursa yalnız keçidi verir (link-only aktivasiya)", () => {
    ...
    expect(found!.link).toBe("https://nexora.example/a?t=zz9");
  });
